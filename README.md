...
Each category defines an ordered **drilldown path**: a sequence of `category segment`s narrowing from the broadest identity scope to the most specific. The tables below list each category's `category segment`s in that order.

A `PAC-ID` MAY stop at any point along the path. Every such prefix is a valid identity in its own right, at a correspondingly broader scope — a `-MD` carrying only `240` identifies a device *model*, whereas `240` followed by `21` identifies one particular device. Any of these scopes MAY be used to attach data.

- At least one `category segment` MUST be present.
- The `category segment`s that are present MUST appear in the order defined by the category.
- `category segment`s MAY be omitted from within the path. Where one is omitted, every following `category segment` MUST carry an explicit `id segment key` (see [Short Notation](#short-notation)).
- If needed, `custom segment`s MAY be added. If so, they MUST be placed after the category's `category segment`s.
...
The short notation omits the keys for segments of each category. Keys are implicitly assigned by walking the category's drilldown order, until an explicit key that differs is reached or an `id segment` starting with `-` is reached. Explicit keys MAY be used along implicit ones, as long as the order of segments is matched.

This is why the order of `category segment`s is normative rather than advisory: implicit key assignment is only unambiguous if the segments present follow the drilldown order.
...
| Beehive of the ACME company. Tracked in Fluidics360 Asset Management | `HTTPS://PAC.ACME.COM/-MD/240:BEEHIVE/21:1234/-PS/240:FLUIDICS360` | `1234` is the asset number assigned by SAP. |
...
The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in BCP 14 [RFC 2119](https://www.ietf.org/rfc/rfc2119.txt) [RFC 8174](https://www.ietf.org/rfc/rfc8174.txt) when, and only when, they appear in all capitals, as shown here.
